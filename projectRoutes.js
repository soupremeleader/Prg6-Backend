...
routes.post("/", (req, res, next) => {
    if(req.header("Accept") === "application/json" || "application/x-www-form-urlencoded"){
        next();
    } else {
        res.status(415).send();
    }
...
routes.put('/:id', async ({params, body}, res) => {
    const {id: _id} = params;
    const {title: title, type: type, size: size, wool: wool, row: row, link: link} = body;

    if (typeof(title) === "undefined" || typeof(type) === "undefined" || typeof(size) === 'undefined' || typeof(wool) === 'undefined' || typeof(row) === 'undefined'|| typeof(link) === 'undefined'  
    || title === "" || type === "" || size === "" || wool === "" || row === "" || link === "") {
        res.status(400).json('title, type, size, wool, row and link cannot be empty')
        return;
    }
    const newProject = {
        title, 
